...
def main():
    ...
    paramsFinal, J_history = gradient_descent(initial_parameters, Y, R, num_users, num_movies, num_features, 0.001, 400,
                                              Lambda)
    ...
    # Ynorm, Ymean = normalize_ratings(Y, R)
    # user_ratings_mean = Ymean

    # R_demeaned = R - user_ratings_mean.reshape(-1, 1)

    # U, sigma, Vt = svds(R_demeaned, k=300)

    U, sigma, Vt = svds(Y - Ymean.reshape(-1, 1).T, k=300)
    ...
    for i in all_user_predicted_ratings[943].argsort()[-15:][::-1]:
        print("Predicting rating", round(float(all_user_predicted_ratings[943][i]), 1), "index: ", movieList[i])

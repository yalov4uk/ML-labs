...
def main():
    ...
    assert process_email(
        "<i class=\"italic\"><p><b>Hello world</b><span>Ola</span></p></i>") == "hello world ola", 'Not implemented removing html-tags functional'
    assert process_email(
        "http://www.leningrad.spb.ru") == "htthttpaddr", 'Not implemented replacing http addresses functional'
    assert process_email(
        "john.smith@gmail.com") == "emailaddr", 'Not implemented replacing email addresses functional'
    ...
    # 21
    email_sample1 = open('emailSample1.txt', "r").read()
    ...
    # 22
    spam_sample = "Hi, John Smith, Items on your wishlist are now discounted! Some items you've added to your personal wishlist are currently discounted on http://www.gog.com/ - you can find all the details below or directly on your https://www.gog.com/account/wishlist."
    spam_sample = transform_email_to_features(spam_sample, vocabList_d)
    print('\n Gaussian Kernel: ')
    print('Spam', classifier6.predict(spam_sample.T))

    email_sample = "Hi John Smith, Your job search status is open, but not actively looking, but we need more information before accelerating your matches. Once you tell us just a little bit more about yourself, we'll get you in front of companies and send you any new jobs that match your interests."
    email_sample = transform_email_to_features(email_sample, vocabList_d)
    print('Email (should be false - at least gmail doesn\'t classify this message as a spam)',
          classifier6.predict(email_sample.T))
